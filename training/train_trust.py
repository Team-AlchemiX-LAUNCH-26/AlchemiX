...
- Compares baseline, ridge, random forest, gradient boosting, and
  HistGradientBoosting. The Go runtime export remains HistGradientBoosting-compatible.
"""
...
JOBLIB_PATH = os.path.join(
    os.path.dirname(__file__), "..", "internal", "models", "trust_model.joblib"
)
...
def main() -> None:
    ...
    train = build_trust_features(preprocessor.transform(raw_split.train))
    val = build_trust_features(preprocessor.transform(raw_split.val))
    test = build_trust_features(preprocessor.transform(raw_split.test))

    # Drop rows where under_report_ratio target is NaN (measured_latency_ms == 0).
    train = train.dropna(subset=["under_report_ratio"])
    val = val.dropna(subset=["under_report_ratio"])
    test = test.dropna(subset=["under_report_ratio"])
    print(
        "  Rows after dropping invalid targets: "
    ...
    # Model comparison across suitable regression families. The Go runtime
    # currently consumes HGB tree JSON, so we export the best HGB model while
    # recording the broader validation comparison.
